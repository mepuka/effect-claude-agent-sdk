...
function make() {
  ...
  const connect = Effect.fn("SyncService.connect")(function*(
    ...
    yield* runTracked(key, log.registerRemote(remote))
  })
  ...
  const connectWebSocket = Effect.fn("SyncService.connectWebSocket")(function*(
    ...
    yield* markConnected(url)
    yield* runTracked(
      url,
      EventLogRemote.fromWebSocket(url, options).pipe(
  ...
  const syncNow = Effect.fn("SyncService.syncNow")(() => Effect.void)
